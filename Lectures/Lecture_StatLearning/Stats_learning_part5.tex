...
\begin{frame}{Linear Discriminant Analysis (Case 1) cont}
	For example:
	\begin{itemize}
		\item $\hat{\pi}_1=0.7$, $\hat{\pi}_2=0.3$
		\item The sample variance $\hat{\sigma}^2=0.5$
		\item Means are $\hat{\mu_1}=-5$ and $\hat{\mu_2}=3$.
		\item The decision boundary is $x=\frac{(\hat{\mu}_1^2-\hat{\mu}_2^2)/2\hat{\sigma}^2 +\log(\hat{\pi}_2/\hat{\pi}_1)}{(\hat{\mu}_1-\hat{\mu}_2)/\hat{\sigma}^2}\approx-3.15$
	\end{itemize}
