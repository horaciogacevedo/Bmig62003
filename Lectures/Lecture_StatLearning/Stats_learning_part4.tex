...
\begin{alertblock}{Logistic regression and classification}
Logistic regression provides a probability model, using it for classification  means that you know something about the outcome beyond your data.
\end{alertblock}

 Thus, if $p(X=x)>0.5$ we can say that $X=x$ belongs to the class 1. However, the cutoff used is something that requires justification. 
...
However, when we use such a model we may encounter negative probabilities!
		...
		\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{../../Figures/fig_logreg.png}
	\end{figure}
\end{frame}
...
\begin{frame}{Support Vectors}
	...
	An important property is that the maximal margin hyperplane depends directly on only a small subset of the observations (the support vectors).
		...
		\begin{figure}[h]
		\centering
		\includegraphics[scale=0.3]{../../Figures/fig_support_vectors.png}
	\end{figure}	
\end{frame}
...
\begin{itemize}
	\item Logistic regression is a probabilistic model. The use as a classification tool requires knowledge beyond what data can provide.
