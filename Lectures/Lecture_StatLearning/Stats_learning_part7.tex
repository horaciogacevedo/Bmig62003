...
\begin{frame}{Regression Trees (cont)}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.4]{../../Figures/fig_hitters_graph.png}
	\end{figure}
...
\begin{frame}{Normalized Penalty CP parameter}
	...
	In Python (Scikit-Learn) has a cost complexity pruning parameter labeled as \textbf{cpp\_alpha}.
\end{frame}
...
\begin{frame}{Surrogate Splits CART (cont)}
	...
	\end{equation*}
	This measure is the relative reduction in error obtained by using $\tilde{s}_m$ to predict $s^*$ as compared with the $\max (p_L,p_R)$. If $\lambda(s^*| \tilde{s}_m)\le 0$, $\tilde{s}_m$ is no help in predicting $s^*$ and is discarded as a surrogate split. 
...
\begin{frame}{Variable Ranking CART}
	Let $T$ be an optimal subtree selected by the cross-validation or test sample procedure. If the Gini splitting rule has been used, then at each node compute $\Delta I(\tilde{s}_m,t)= I(t)-I(t_L)-I(t_R)$, where $I$ is the impurity function.
