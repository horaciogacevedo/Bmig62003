...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{../../Figures/fig_threshold.png}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.8]{../../Figures/fig_keras_linear_reg.jpg}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{../../Figures/fig_mactivation1.png}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{../../Figures/fig_mactivation2}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{../../Figures/fig_mactivation3}
\end{figure}
\end{frame}
...
\begin{frame}{Hockey stick functions}
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.5]{../../Figures/fig_mactivation4}
	\end{figure}
	\begin{enumerate}
				\setcounter{enumi}{1}
		\item Parametric Rectified Linear Unit (PReLU). In this case the activation is piecewise linear, having different firing rates for $x<0$ and $x>0$
		\begin{equation*}
...
\end{equation*}		
	...
	\end{enumerate}
\end{frame}
\begin{frame}{Hockey stick functions (cont)}

	\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{../../Figures/fig_mactivation5}
\end{figure}
\end{frame}
...
These types of activation functions have that advantage that they are smooth and can approximate a step function to any degree of accuracy.
	...
	\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{../../Figures/fig_mactivation6}
\end{figure}
\end{frame}
...
\begin{frame}{Sigmoid Functions}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{../../Figures/fig_mactivation7}
\end{figure}
\end{frame}
...
\begin{frame}{Sigmoid Functions}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{../../Figures/fig_mactivation8}
\end{figure}
\end{frame}
...
\begin{frame}{Input, Output, and Target (cont)}
	...
	The \textbf{target function} is the desired functional relationship that the network tries to approximate. This function is independent of the parameters $w$ and will be denoted by $z=\phi(x)$, $\textbf{z}=\phi(\textbf{x})$ or $Z=\phi(X)$.  
...
\begin{frame}{Goal}
	...
	The proximity will be determined by the cost function 
	\begin{equation*}
		C(w,b)=\text{distance}(y,z)
	\end{equation*}
...
\begin{frame}{Cross-entropy (cont)}
For example let $p(x)= \exp(-x^2/2)/\sqrt{2\pi}$ be the density of the $N(0,1)$, and $q(x)=\exp(-x^2/50)/(5*\sqrt{2\pi})$ be the corresponding density for $N(0,5)$. 
\begin{equation*}
	S(p,q)= 13.4189 \textrm{ whereas } S(q,p)=2.54838
\end{equation*}
	...
	With the same notation we have 
		\begin{equation*}
		H(p)= 1.41894 \textrm{ whereas } H(q)=3.02838
	\end{equation*}
...
As we said, the KL divergence gives us a mechanism to measure how similar two distributions are. The \textbf{mutual information} between two random distributions gives us an idea of how dependant two random variables are.


\begin{equation*}
I(p;q)= D_{KL}(p(x,y),p(x)p(y))=H(p)-H(p|q)=H(q)-H(q|p)
\end{equation*}
The value will be zero when $p(x,y)=p(x)p(y)$ that means that the variables are independent.
\end{frame}
...
\begin{frame}{References}
	...
	I have used some of the graphs by hacking TiKz code from StakExchange, Inkscape for more aesthetic plots and other old tricks of \TeX. The entropy calculations were done in Mathematica.
