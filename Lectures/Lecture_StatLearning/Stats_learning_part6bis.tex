...
\end{equation*}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[width=10cm]{../../Figures/fig_manifold_basic_idea.jpg}
\end{figure}	
\end{frame}
...
\begin{frame}{MDS solution}
%	Using matrix notation, we have 
%	\begin{equation*}
%		\tilde{\textbf{X}}= \textbf{C}_N \textbf{X} (\textbf{C}_N \textbf{X} )^t
%	\end{equation*}
	If we were able to identify a centered matrix $\tilde{\textbf{Z}}=\{\tilde{Z}_{ij}\}$
...
such that the cost function $\cal{E}$ is minimized
	\begin{equation*}
		{\cal E}=\| \tilde{\textbf{K}}- \tilde{\textbf{Z}} \|^2=\sum_{i,j} ( \tilde{K}_{ij} - \tilde{Z}_{ij})^2 
	\end{equation*}
	
	How to find such a matrix $\tilde{Z}$? 
	
	If we apply the \textbf{singular value decomposition} of $\tilde{\textbf{K}}$. Thus, $\tilde{\textbf{K}}=\textbf{U} \textbf{S} \textbf{V}^t$ and $S=\text{diag}(s_1,\ldots,s_D)$ and all of them are non negative and ordered in descending value.
	Thus, instead of using the whole matrix $\textbf{S}$, we just take the first two components $(s_1,s_2)$, we take their square roots and generate
\begin{equation*}
\tilde{\textbf{Z}}= \textbf{U} \text{diag}(\sqrt{s_1},\sqrt{s_2},0,\cdots,0)
\end{equation*}
\end{frame}
