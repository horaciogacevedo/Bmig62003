...
\begin{document}
	...
	\begin{frame}{What is a Hypothesis test?}
		\begin{itemize}
...
\begin{frame}{Example 2(cont)}
	...
	\begin{equation*}
		\begin{split}
			1-\beta(\tilde{\mu})&= 1-P^{\tilde{\mu}}_{X_1,X_2}(G^c)= P^{\tilde{\mu}}_{X_1,X_2}(G)= P^{\tilde{\mu}}((X_1,X_2)\in G)\\
			&=P(X_1 \le 494.63 \text{ and } X_2 \le 494.63| \mu=\tilde{\mu}, \sigma^2=50) \\
			&=P(X_1 \le 494.63 | \mu=\tilde{\mu}, \sigma^2=50)\\
			&\quad  \cdot P(X_2\le 494.63 | \mu=\tilde{\mu}, \sigma^2=50) 
		\end{split}
	\end{equation*}
\end{frame}
...
\begin{frame}{Power function example}
	\begin{figure}[h]
	\centering
	\includegraphics[scale=1]{../../Figures/fig_power_function_ink.jpg}
\end{figure}	
\end{frame}
...
\begin{frame}{$p$-values}
	We were familiar from basic statistics courses about the normally distributed instances. But from the previous discussion we can see that the same ideas can expand to accommodate
	
	\begin{itemize}
		\item different critical regions,
		\item different test statistics
	\end{itemize}
\end{frame}
...
\begin{frame}{Ajusted $p$-values}
	...
	Note that we assume that the individual tests are independent of each other.  
\end{frame}

\begin{frame}{Bonferroni correction}
	There are some issues with the Bonferroni's correction. 
	
	\begin{itemize}
		\item In genomics, if you have 20,000 gene transcripts results for both controls and a certain disease. The Bonferroni correction a $p$-value would have to be less than 0.05/20000=$2.5\times10^{-6}$. Your results might have a large number of false negatives from transcripts that might have been biologically important.
		\item Another issue is with the definition of a "family", so if you have a model with certain $n_1$ dietary variables and $n_2$ socioeconomic variables. The critical value should be $0.05/n_1$ or $0.05/(n_1+n_2)$?
	\end{itemize}
\end{frame}

\begin{frame}{FDR control}
	The Benjamini and Hocheberg procedure first puts all $p$-values in order from smallest to largest and ranks them. The critical value is defined as
	\begin{equation*}
		(i/m)Q,
	\end{equation*}
	where $i$ is the rank, $m$ is the total number of tests, and $Q$ is the \textbf{False Discovery Rate} that the user picks.
...
\begin{frame}{Correct or not correct that is the question...}
	Whereas we aim use multiple comparison corrections to reduce the number of false positives, we also increase the number of false negatives! 
	
	If false negatives are very costly (not only financially but scientifically), then you may decide not to correct for multiple comparison.
	
	For instance, if you have a small animal experiment using transcriptomics for the comparison of a case and control. If your conditions are very stringent for multiple comparison adjustments, you may not be able to find other possible interactions that may be occurring. Sometimes, you cannot even conduct a "pathway analysis" if you have very few hits. 
	 
\end{frame}

\begin{frame}{References}
	Materials and some of the pictures are from \citep{pestman}, \citep{macdonald} and \citep{speed}.
	\printbibliography 	
