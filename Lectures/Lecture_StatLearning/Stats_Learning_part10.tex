...
\begin{frame}{Line search method}
	...
	A variant of the gradient descent with an adjustable learning rate $\eta$ is the \textbf{line search method}.
	\begin{enumerate}
		\item Start with an initial point $x^0$ and consider the normal direction of the level hypersuface $S_{f(x^0)}$, which is given by the gradient $\nabla f(x^0)$.
		\item Pick a point $x^1$ on the line given by this direction at which the objective function reaches a minimum. This is equivalent to find $\eta_0$ such that
		\begin{equation*}
			\eta_0=\text{argmin}_\eta f(x^0 - \eta \nabla f(x^0))
		\end{equation*}
		\item Define $x^1=x^0-\eta_0 \nabla f(x^0)$ and repeat the process.
	\end{enumerate} 
\end{frame}
\begin{frame}{Line search method}
	\begin{figure}[h]
		\centering
		\includegraphics{../../Figures/fig_line_search.png}
	\end{figure}
...
\begin{frame}{RMSProp}
	 The \textbf{Root Mean Square Propagation} or RMSProp is a variant of the gradient descent method with adaptive learning, which is obtained when the gradient is divided by a running average of its magnitude.
